...
鼠标**左键点击** 2D 热力图任意位置，弹出独立深色公式窗口，逐行展示：

| 工作区 | 显示内容 |
|--------|----------|
| **Cutoff** | `V_GS < V_TH` → 沟道未形成，`I_D ≈ 0` |
| **Subthreshold** | `V_GS ≲ V_TH` → Level 1 模型 `I_D ≈ 0` |
| **Deep Triode** | `I_D ≈ β·V_ov·V_DS` 近似式；**Ron 公式 + 代入值**（`R_on = V_DS/I_D ≈ 1/(β·V_ov)`）；gm |
| **Triode** | 完整 Gismondi 公式 `I_D = β·[V_ov·V_DS,eff − ½·V_DS,eff²]·(1+λ·V_DS)` + 逐步代入计算 |
| **Saturation** | 经典 `I_D ≈ ½β·V_ov²·(1+λ·V_DS)` + 逐步代入 + Gismondi 精确值对比；gm |

每弹窗包含：β 值推导、`V_ov`、`V_DS,eff`（Gismondi 平滑中间量）、中间计算步骤、最终 `I_D`、`g_m`。重复点击自动关闭旧窗重开。

### 2. 点击蓝色标记

点击点同时在 2D 图留下**蓝色方块**标记和**坐标标注**，在 3D 图留下**蓝色菱形**，与悬停红色高亮区分。

---

## Bug 修复

### B1 — Windows GBK 终端编码（v1 + v2）
- **现象**：`²`（上标2）、`⁻³` 等 Unicode 字符在 Windows GBK 终端下抛出 `UnicodeEncodeError`
- **修复**：文件头部添加 `sys.stdout.reconfigure(encoding="utf-8")`（v1.py + v2.py 均已修复）

### B2 — mathtext `\textcolor` 不支持（v2）
- **现象**：公式弹窗 header 使用了 `\textcolor{#xxx}{...}` 导致 mathtext 解析崩溃
- **修复**：header 改为纯文本，颜色由 `fig.text(color=...)` 控制

### B3 — mathtext `\text{-}` 不支持（v2）
- **现象**：`\text{-}` 在 matplotlib 3.5.3 的 mathtext 中不被识别
- **修复**：`\text{-}` → `-`（普通连字符）

### B4 — Triode 区 gm 计算缺因子（v2）
- **现象**：公式弹窗和悬停信息框中 Triode 区 gm 显示公式为 `g_m = β·V_DS·(1+λ·V_DS)`，但数值计算为 `β·V_DS`（缺少 `(1+λ·V_DS)` 因子）
- **修复**：`gm = beta_mA * vds_val * (1.0 + lam * vds_val)`，公式与数值一致
